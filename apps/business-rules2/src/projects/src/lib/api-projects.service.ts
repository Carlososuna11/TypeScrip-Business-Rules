import { ApiUploadDataService } from '@business-rules22/business-rules2/src/upload-data';
...
export class ProjectsService {
  ...
  async updateProject(id: string, body: ProjectsValidation) {
    ...
    return await this.projectsRepository
      .query(`
      UPDATE projects SET name= '${body.name}', description= '${body.description}', file= 'tsbr/${nameTable}' WHERE id = ${id}`)
      .then(
        function (user) {
          console.log(user);
          return user;

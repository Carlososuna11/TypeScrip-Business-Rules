...
export class ProjectsController {
  ...
  @Put(URLS.updateStatusProject)
  async updateStatusProject(@Param('id') id: string,@Body() body: ProjectsUpdateStatusValidation, @Query() query, @Res() res: Response, @Headers() headers) {
    await this.ProjectsService.updateStatusProject(id, body)
    .then((result) => {
      const response = new ResponseService(STATUS.success, result, MESSAGES.success);
      res.status(STATUS.success).send(response);
    })
